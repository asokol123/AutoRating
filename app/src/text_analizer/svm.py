...
class SVMSentiment(ModelBase):
    def __init__(self, loss: str='log') -> None:
        super().__init__()
        self.pipeline = Pipeline(
            [
                ('Vectorize', CountVectorizer()),
                ('TF-IDF', TfidfTransformer()),
                ('Classify', SGDClassifier(loss=loss)),
            ]
        )
        self.learner = None
    ...
    def predict_str(self, text: str) -> int:
        return self.learner.predict([text])
...
class SVMSentimentSum(ModelBase):
    def __init__(self, loss: str='log') -> None:
        super().__init__()
        self.pipeline = Pipeline(
            [
